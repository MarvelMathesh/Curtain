...
export default function AdminPage() {
  ...
  const createVenue = async (e: React.FormEvent) => {
    ...
    else { setMsg(`Created ${j.venue.name} - ${j.venue.rows}×${j.venue.cols} (${j.venue.seats.length} seats)`); load() }
  }
  ...
  const reset = async () => {
    ...
    else { setMsg(`Reset - ${j.venues} venues, ${j.events} events`); load() }
  }
    ...
    <div className="relative min-h-screen">
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-24 sm:pt-28 pb-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <Button variant="destructive" size="sm" onClick={reset} className="rounded-full">Reset DB to seed</Button>
        </div>
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[0.95fr_1.05fr]">
          <Card className="p-6 gap-4 self-start">
            ...
            <form onSubmit={createVenue} className="space-y-3">
              <div><Label>Name</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="Curtain Black Box" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>City</Label><Input value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} required placeholder="Mumbai" /></div>
                <div><Label>Address</Label><Input value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} placeholder="Bandra, Mumbai" /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Rows (1-20)</Label><Input type="number" min={1} max={20} value={form.rows} onChange={(e) => setForm({ ...form, rows: e.target.value })} required /></div>
                <div><Label>Cols (1-20)</Label><Input type="number" min={1} max={20} value={form.cols} onChange={(e) => setForm({ ...form, cols: e.target.value })} required /></div>
              </div>
              <div><Label>Image URL (optional)</Label><Input value={form.image} onChange={(e) => setForm({ ...form, image: e.target.value })} placeholder="https://images.unsplash.com/…" /></div>
              <Button type="submit" className="w-full rounded-lg bg-gradient-to-r from-[oklch(0.646_0.222_41.116)] to-[oklch(0.488_0.243_264.376)] text-white">Create venue</Button>
                  ...
                  <Card key={v.id} className="overflow-hidden p-0 gap-0">
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-2 flex gap-2 text-xs">
                        <span className="rounded-full bg-amber-50 border border-amber-200 px-2.5 py-1">Premium - first 2 rows</span>
                        <span className="rounded-full bg-violet-50 border border-violet-200 px-2.5 py-1">Standard - next 3 rows</span>
                        <span className="rounded-full bg-sky-50 border border-sky-200 px-2.5 py-1">Economy - rest</span>
                      </div>
                    </div>
                  </Card>
                ))}
